// table.js - Versió amb hora de l'actualització
let previousValues = {};
        ...
        .then(results => {
            ...
            Promise.all(municipis.map(m => fetch(getHistoricalUrl(m.lat, m.lon, startDate, endDate)).then(r => r.json())))
                .then(histResults => {
                    ...
                    // Hora de l'actualització (NOVA FILA AL COMIAT)
                    html += `<tr>
                    ...
                    // Abans de marcar els nous canvis, netegem les classes anteriors
                    ...
                    // Actualitza previousValues per a totes les dades (EXCEPTE l'hora de l'actualització)
                    previousValues = {
                    ...
                    // Guardem també l'hora de l'actualització per si volem utilitzar-la
...
function clearPreviousHighlights() {
    // Eliminem les classes 'up' i 'down' de totes les cel·les de la taula
...
function formatUpdateTime(timeString) {
    ...
    // El format és "2023-10-05T14:00" o similar
    try {
        const date = new Date(timeString);

        // Convertim a hora local
        ...
        return timeString; // Retorna l'string original si hi ha error
    }
}

// Funció per marcar canvis - MODIFICADA: sense setTimeout
function checkAndMarkChange(id, newValue) {
    ...
    if (newValue > prev) {
        ...
        // ELIMINAT: setTimeout(() => cell.classList.remove('up'), 50000);
    } else if (newValue < prev) {
        cell.classList.add('down');
        // ELIMINAT: setTimeout(() => cell.classList.remove('down'), 50000);
    }
}
